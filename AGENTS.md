...
## 11. Кросс-проектная документация

Общая документация всего VPN-сервиса вынесена в **отдельный репозиторий [`vpn-docs`](https://github.com/PavelPotapov/vpn-docs)** (клонируется рядом). Точка входа — `vpn-docs/PROJECT_STATE.md` (модули, матрица фич, флоу, пробелы к релизу); `vpn-docs/security/ACCESS_CONTROL_DESIGN.md`, `vpn-docs/billing/`, `vpn-docs/release/`. Читать при вопросах об общем состоянии сервиса и связи сайта с бэкендом/клиентом.
